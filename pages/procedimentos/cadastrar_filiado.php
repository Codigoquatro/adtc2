...
$arquivo=$_FILES['arquivo']['name'];
			...
			$_UP['pasta'] = '/home2/tcmarang/public_html/sistema/imagens/';
...
$sql = "SELECT * FROM filiado WHERE documento ='$documento' ";
$sql = $pdo->query($sql);
...
$sql = "INSERT INTO filiado SET nome='$nome',funcao='$funcao',congregacao='$congregacao',documento='$documento',dataNascimento='$dataNascimento',dataBatismo='$dataBatismo',estadoCivil='$estadoCivil',mae='$mae',pai='$pai',logradouro='$logradouro',endereco='$endereco',numero='$numero',bairro='$bairro',cep='$cep',cidade='$cidade',uf='$uf',telefone='$telefone',email='$email',status='$status',datCadastro='$data',arquivo='$arquivo'";
 $sql = $pdo->query($sql);
...
<?php
		if ($sql->rowCount() >0 ){	
			...
			";		   
				...
				<META HTTP-EQUIV=REFRESH CONTENT = '0;URL=../view/tela_cad_filiado.php'>
				<script type=\"text/javascript\">
					alert(\"Filiado não foi cadastrado !!.\");
				</script>
			";		   
