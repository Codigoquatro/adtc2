...
<?php 
 ...
 $sql=$pdo->query($sql);
  ...
  $sql = "SELECT * FROM patrimonio WHERE id_patrimonio ='$id_patrimonio' LIMIT 1";
  $sql=$pdo->query($sql);
  if ($sql->rowCount() > 0) {
    # code...
...
<body>
  ...
  <div class="container">
   <form novalidate class="needs-validation" action="procedimento_alterar_patrimonio.php"  method="POST" enctype="multipart/form-data">
   <div class="form-row">        
        <div class="col form-group">          
           <input type="hidden" name="id_patrimonio" class="form-control is-valid" id="id_patrimonio" value="<?php echo $linhas['id_patrimonio']; ?>" >
        </div>
    </div><!-- end div-form-row--> 
    <div class="form-row">        
        <div class="col form-group">
           <label for="nome">Descrição</label>
           <input type="text" name="descricao" class="form-control is-valid" id="descricao" value="<?php echo $linhas['descricao']; ?>"   required>
        </div>
    </div><!-- end div-form-row-->    
    <div class="form-row">     
        <div class="col form-group">       
        <label for="nome">Quantidade</label>
           <input type="text" name="qtde" class="form-control is-valid" id="qtde" value="<?php echo $linhas['qtde']; ?>" required>
        </div>
    </div><!-- end div-form-row-->
